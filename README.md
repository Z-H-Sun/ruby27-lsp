...
- You will need to alter the extension `js` file. The easiest way is to go to `%UserProfile%` (or equivalent), and then open `/.vscode/extensions/shopify.ruby-lsp-0.10.0/out/extension.js`, and then
  ...
  - also, to disable the auto-update (which nullifies our edits), find and delete the code ``try{await this.installOrUpdateServer(!1)}catch(t){this.error=!0,await wt.window.showErrorMessage(`Failed to setup the bundle: ${t.message}.         See [Troubleshooting](https://shopify.github.io/ruby-lsp/troubleshooting.html) for help`);return}``
